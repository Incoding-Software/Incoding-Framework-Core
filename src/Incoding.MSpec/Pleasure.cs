...
namespace Incoding.MSpec
{
    ...
    public static class Pleasure
    {
        ...
        public static class Generator
        {
            ...
            public static MemoryFormFile MemoryFormFile(Stream stream = null, string fileName = "fileName", string contentType = "contentType")
            {
                if (stream == null)
                    stream = Stream();

                return new MemoryFormFile(stream, fileName, contentType);
            }
            

            public static IFormFile FormFile(Stream stream = null, string fileName = "fileName", string contentType = "contentType")
            {
                if (stream == null)
                    stream = Stream();

                return MockAsObject<FormFile>(mock =>
                                                            {
                                                                mock.SetupGet(r => r.Length).Returns((int)stream.Length);
