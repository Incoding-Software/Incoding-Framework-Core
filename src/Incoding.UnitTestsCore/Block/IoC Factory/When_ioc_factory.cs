...
namespace Incoding.UnitTest.Block
{
    ...
    public class When_ioc_factory
    {
        ...
        protected static Mock<IServiceProvider> iocProvider;
        ...
        Establish establish = () =>
                                  {
                                      iocProvider = new Mock<IServiceProvider>();
                                      iocFactory = new IoCFactory();
                                  };

        Because of = () => iocFactory.Initialize(iocProvider.Object);

        It should_be_get_by_type = () =>
                                       {
                                           iocProvider.Setup(r => r.GetService(typeof(ILogger))).Returns(new ClipboardLogger());
        ...
        It should_be_try_resolve = () =>
                                       {
                                           iocProvider.Setup(r => r.GetService(typeof(ILogger))).Returns((ILogger)null);
                                           iocFactory.TryResolve<ILogger>().ShouldBeNull();
                                       };

        It should_be_resolve_all = () =>
                                       {
                                           iocProvider.Setup(r => r.GetServices<ILogger>()).Returns(new List<ILogger> { new ClipboardLogger(), new ClipboardLogger() });
                                           iocFactory.ResolveAll<ILogger>().Count().ShouldEqual(2);
                                       };

        It should_be_try_get_by_named = () =>
                                            {
                                                iocProvider.Setup(r => r.GetService<ILogger>(Pleasure.Generator.TheSameString())).Returns(new ClipboardLogger());
