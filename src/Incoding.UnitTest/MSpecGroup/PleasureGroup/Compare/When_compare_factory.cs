namespace Incoding.UnitTest.MSpecGroup
{
    ...
    public class When_compare_factory
    {
        ...
        It should_be_compare_type = () =>
                                    {
                                        var compare = new CompareFactory<Type, Type>();
                                        compare.Compare(typeof(string), typeof(string));
                                        compare.IsCompare().ShouldBeTrue();
                                    };

        It should_be_not_compare_type = () =>
                                        {
                                            var compare = new CompareFactory<Type, Type>();
                                            compare.Compare(typeof(string), typeof(int));
                                            compare.IsCompare().ShouldBeFalse();
                                        };
        ...
        It should_be_compare_with_enum_as_flag_false_with_multiple = () =>
                                                                     {
                                                                         var compare = new CompareFactory<FakeCompareWithFlag, FakeCompareWithFlag>();
                                                                         compare.Compare(new FakeCompareWithFlag { OfType = OfType.Line | OfType.DownVote }, new FakeCompareWithFlag { OfType = OfType.DownVote });
                                                                         compare.IsCompare().ShouldBeFalse();
        ...
        It should_be_compare_with_forward_to_string_false = () =>
                                                            {
                                                                var compare = new CompareFactory<FakeCompare, FakeCompareWithGuid>();
                                                                compare.ForwardToString(r => r.Result);
                                                                compare.Compare(new FakeCompare { Result = Guid.NewGuid().ToString() }, new FakeCompareWithGuid { Result = Guid.NewGuid() });
                                                                compare.IsCompare().ShouldBeFalse();
                                                            };

        It should_be_compare_with_forward_to_string_with_null = () =>
                                                                {
                                                                    var compare = new CompareFactory<FakeCompare, FakeCompareWithObject>();
                                                                    compare.ForwardToString(r => r.Result);
                                                                    var theSameGuid = Pleasure.Generator.TheSameGuid();
                                                                    compare.Compare(new FakeCompare { Result = theSameGuid.ToString() }, new FakeCompareWithObject { Result = null });
                                                                    compare.IsCompare().ShouldBeFalse();
                                                                };
